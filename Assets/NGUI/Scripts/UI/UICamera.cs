...
public class UICamera : MonoBehaviour
{
	...
	static public GetAxisFunc GetAxis = m_GetAxisFunc;

    static float m_GetAxisFunc(string name)
    {
        float ret = Input.GetAxis(name);
        if (name == "Mouse ScrollWheel")
        {
            if (ret == 0)
            {
                ret = Program._padScroll;
            }
        }
        ret *= Program.deltaTime * 300;
        return ret; 
    }
